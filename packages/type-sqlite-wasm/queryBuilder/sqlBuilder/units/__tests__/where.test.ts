...
describe('whereUnit', () => {
    const testCases: Array<{
        ...
        expected: [string, any[]]
            ...
            },
            {
                name: '多个AND条件',
                input: [
                    { rule: { condition: { age: { $gte: 18 }, status: 'active' }, type: 'AND' } }
                ],
            ...
            },
            {
                name: '复杂IN条件',
                input: [
                    { rule: { condition: { status: { $in: ['active', 'pending'] } }, type: 'AND' } }
                ],
            ...
            },
            {
                ...
                expected: ['WHERE age >= ? AND age <= ? AND status = ? OR score > ?', [18, 60, 'active', 80]]
            },
            {
                name: 'Raw SQL条件',
                input: [
    ...
    test('错误处理 - 条件数量超限', () => {
        const input: WhereClause[] = Array(1001).fill(0).map((_, i) => ({
